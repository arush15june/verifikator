...
class SigInfo(object):
    ...
    def __init__(self, signature_filename, *args, **kwargs):
        ...
        # self.dataset_type = kwargs.get('dataset_type', self.DEFAULT_DATASET_TYPE)
        self.dataset_type = 'sample'
        # Set metadata
        self._set_sig_metadata(signature_filename)
        
    def __repr__(self):
        return "<SigInfo {}{} {}>".format(
            str(self.signature).zfill(3),
            str(self.sample).zfill(2),
            'G' if self.genuine else 'F',
        )

    def _set_sig_metadata(self, signature_filename):
        if self.dataset_type == 'sample':
            self._sample_set_metadata(signature_filename)
        elif self.dataset_type == 'cedar':
            self._cedar_set_metadata(signature_filename)
                
    def _sample_set_metadata(self, signature_filename):
        self.signature = signature_filename[0:3]
        self.sample = signature_filename[3:5]
        self.done_by = signature_filename[5:8]

        if(self.signature == self.done_by):
            self.genuine = True
            self.forged = False

    def _cedar_set_metadata(self, signature_filename):
